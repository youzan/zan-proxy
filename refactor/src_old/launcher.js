...
class Launcher {
     ...
     * @param port
     */
    async start(pluginManager) {
        await this._initService();
        await this._startProxyServer();
        await this._startWebUiServer(pluginManager);
    ...
    // 启动管理界面服务器
    async _startWebUiServer(pluginManager) {
        ...
        // 启动web ui
        await new WebUiServer(webUiPort, pluginManager).start();

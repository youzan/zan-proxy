...
module.exports = class CertificationService {
    ...
    async start() {
        // 读取根证书
        let appDir = this.appInfoService.getProxyDataDir();

        let key = await fileUtils.readFile(path.join(appDir, 'certificate/root/zproxy.key.pem'));
        let cert = await fileUtils.readFile(path.join(appDir, 'certificate/root/zproxy.crt.pem'));
        this.root = {
            cert,
            key
        };

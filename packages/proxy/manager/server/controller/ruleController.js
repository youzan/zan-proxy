...
const { ProfileConfigServiceToken, RuleServiceToken } = require('lib-zan-proxy/lib/service')
...
module.exports = class RuleController {
    ...
    constructor() {
        ...
        this.configService = Container.get(ProfileConfigServiceToken)
    }
    ...
    regist(router) {
        ...
        router.post('/rule/create', async (ctx, next) => {
            let result = await this.ruleService.createRuleFile(ctx.request.body.name
        ...
        router.get('/rule/deletefile', async (ctx, next) => {
        ...
        router.get('/rule/setfilecheckstatus', async (ctx, next) => {
            await this.ruleService.setRuleFileCheckStatus(ctx.query.name,
        ...
        router.get('/rule/getfile', async (ctx, next) => {
            let content = await this.ruleService.getRuleFile(ctx.query.name);
            ctx.body = {
                code: 0,
                data: content
            };
        ...
        router.post('/rule/savefile', async (ctx, next) => {
            await this.ruleService.saveRuleFile(ctx.query.name, ctx.request.body.content);
            ctx.body = {
                code: 0
            };
        });
        ...
        router.get('/rule/download', async (ctx, next) => {
            ...
            let content = await this.ruleService.getRuleFile(name);
        ...
        router.post('/rule/test', async (ctx, next) => {
            ...
            let targetRlt = await this.configService.calcPath(url, match, targetTpl);

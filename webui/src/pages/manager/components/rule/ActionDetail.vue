<template>
  <div style="text-align: left; flex:1;">
    ...
    <div v-show="action.type == 'mockData'" class="inline-block right-panel">
      <div class="action-data">
        ...
        <span v-if="datafileEntry" style="margin-left: 10px;">
          <el-button type="text" @click="editDataFile(datafileEntry)">编辑数据</el-button>
        </span>
    ...
    <div v-if="action.type == 'modifyResponse'" class="inline-block right-panel">
      <div class="action-data">
        <el-select
          v-model="action.data.modifyResponseType"
          style="width: 260px"
          size="small"
        ...
        >
          <el-option
            v-for="item in modifyResponseType"
            :key="item.value"
            :label="item.label"
            :value="item.value"
          ></el-option>
        ...
        <span v-if="action.data.modifyResponseType == 'returnDataInJsonpStyle'">
          <el-input
            v-model="action.data.callbackName"
            size="small"
            :disabled="remote"
            style="width:200px;"
    ...
    <div v-if="action.type == 'scriptModifyRequest'" class="inline-block right-panel">
      <textarea style="width: 500px;height: 90px" v-model="action.data.modifyRequestScript"></textarea>
    </div>
    <!-- 脚本修改响应 -->
    <div v-if="action.type == 'scriptModifyResponse'" class="inline-block right-panel">
      <textarea style="width: 500px;height: 90px" v-model="action.data.modifyResponseScript"></textarea>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      modifyResponseType: [
        {
          value: 'addTimestampToJsCss',
          label: '将html中的js、css请求加上时间戳',
        },

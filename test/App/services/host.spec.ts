...
import { AppInfoService, HostService } from '../../../src/core/services';
...
describe('HostService', () => {
  ...
  it('should resolve the host correctly', async () => {
    const resolved = await hostService.resolveHost('test.youzan.com');
    resolved.should.equal('127.0.0.1');
  });

  it('should not resolve an unknown host', async () => {
    const resolved = await hostService.resolveHost('myfoo.com');
